...
template<typename T>
class miopenCNN {
    TensorDescriptor5d<T> x_desc_;
    TensorDescriptor5d<T> h_desc_;

    FilterDescriptor5d<T> w_desc_;
...
public:
    ...
    miopenCNN(int _w, int _h, int _d, int c, int n, int k, int r, int s, int t,
             int pad_w, int pad_h, int pad_d, int wstride, int hstride, int dstride, Tensor<T> x, Tensor<T> w)
             :
        miopen_handle_(),
        x_desc_(n, c, _d, _h, _w),
        w_desc_(k, c, t, r, s),
	...
	int out_dim = 5;
        ...
        CHECK_MIOPEN_ERROR(miopenGetConvolutionNdForwardOutputDim(conv_desc_.desc(),
                                                                x_desc_.desc(),
                                                                w_desc_.desc(),
                                                                &out_dim,
                                                                &out[0]));
	...
	//printf("n %d c %d d %d h %d w %d \n", out_n, out_c, out_d, out_h, out_w);
        h_desc_ = TensorDescriptor5d<T>(out_n, out_c, out_d, out_h, out_w);
...
std::tuple<int, int, int, std::string> time_cnn(
    ...
    auto filter = rand<T>(std::vector<int>{t, r, s, c, k});

    // Allocate memory for input
    auto input = rand<T>(std::vector<int>{w, h, d, c, n});
    miopenCNN<T> cnn(w, h, d, c, n, k, r, s, t, pad_w, pad_h, pad_d, wstride, hstride, dstride, input, filter);
...
/*    //Allocate memory for backward pass wrt inputs
    auto dX = zeros<T>(std::vector<int>{w, h, d, c, n});
    ...
    for (const auto &problem : inference_server_set) {
        ...
        if( precision == "float" )
        {
            std::tie(fwd_time, bwd_inputs_time, bwd_params_time, fwd_algo_s) =
                time_cnn<float>(k, c, r, s, t, n, h, w, d, pad_h, pad_w, pad_d, hstride, wstride, dstride, num_repeats);
        }
        else
        {
