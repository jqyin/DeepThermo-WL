...
class cudnnCNN {
    TensorDescriptor5d<T1> x_desc_;
    TensorDescriptor5d<T1> h_desc_;

    FilterDescriptor5d<T1> w_desc_;
...
public:
    ...
    cudnnCNN(int w, int h, int d, int c, int n, int k, int r, int s, int t,
             int pad_w, int pad_h, int pad_d, int wstride, int hstride, int dstride,
             int inference)
             :
        cudnn_handle_(),
        conv_desc_(pad_h, pad_w, pad_d, hstride, wstride, dstride)
    {
        int out_h, out_w, out_d, out_c, out_n;
	int out[5];
        ...
        x_desc_ = TensorDescriptor5d<T1>(format, n, c, d, h, w);
        w_desc_ = FilterDescriptor5d<T1>(format, k, c, t, r, s);

#if (CUDNN_MAJOR >= 7) && (USE_TENSOR_CORES)
        cudnnSetConvolutionMathType(conv_desc_.desc(), CUDNN_TENSOR_OP_MATH);
#endif
        // Get output dimensions
        CHECK_CUDNN_ERROR(cudnnGetConvolutionNdForwardOutputDim(conv_desc_.desc(),
                                                                x_desc_.desc(),
                                                                w_desc_.desc(),
								5,
                                                                out));
	...
	out_h = out[3];
	out_w = out[4];
        h_desc_ = TensorDescriptor5d<T1>(format, out_n, out_c, out_d, out_h, out_w);

        output_dims_ = {out_w, out_h, out_d, out_c, out_n};
	//printf("n %d c %d d %d h %d w %d \n", out_n, out_c, out_d, out_h, out_w);
#if USE_GET
        if (std::is_same<T1, uint8_t>::value) {
...
std::tuple<int, int, int, std::string> time_cnn(
         int k, int c, int r, int s, int t,
         int n, int h, int w, int d,
         int pad_h, int pad_w, int pad_d,
         int hstride, int wstride, int dstride,
         int num_repeats,
         curandGenerator_t curand_gen,
         int inference
        ) {

    cudnnCNN<T1, T2> cnn(w, h, d, c, n, k, r, s, t, pad_w, pad_h, pad_d, wstride, hstride, dstride, inference);

    // Allocate memory for filter
    auto filter = rand<T1>(std::vector<int>{t, s, r, c, k}, curand_gen);

    // Allocate memory for input
    auto input = rand<T1>(std::vector<int>{w, h, d, c, n}, curand_gen);
    ...
    if (!inference) {
        ...
        auto dW = zeros<T1>(std::vector<int>{t, s, r, c, k});
        ...
        auto dX = zeros<T1>(std::vector<int>{w, h, d, c, n});
...
int main(int argc, char **argv) {
    ...
    for (const auto &problem : (inference ? inference_server_set : training_set)) {
        ...
        // Filter parameters
        int k, c, r, s, t; // r - filter_h (f_h), s - filter_w (f_w)

        // Input parameters
        int n, w, h, d;

        // Padding
        int pad_w, pad_h, pad_d;

        // Stride
        int wstride, hstride, dstride;

        std::tie(w, h, d, c, n, k, s, r, t, pad_w, pad_h, pad_d, wstride, hstride, dstride) = problem;

        bool skip_kernel = false;
        bool need_padding = false;

#if CUDNN_MAJOR >= 6
        int padded_c, padded_w, padded_h, padded_k, padded_d;
        int pad_value;
        ...
        if (precision == "float") {
            std::tie(fwd_time, bwd_inputs_time, bwd_params_time, fwd_algo_s) =
                time_cnn<float, float>(k, padded_c, r, s, t, n, padded_h, padded_w, padded_d, pad_h, pad_w, pad_d, hstride, wstride, dstride, num_repeats, curand_gen, inference);
        } else if (precision == "half") {
            if (!inference) {
                std::tie(fwd_time, bwd_inputs_time, bwd_params_time, fwd_algo_s) =
                    time_cnn<uint16_t, uint32_t>(padded_k, padded_c, r, s, t, n, padded_h, padded_w, padded_d, pad_h, pad_w, pad_d, hstride, wstride, dstride, num_repeats, curand_gen, inference);
            } else {
                std::tie(fwd_time, bwd_inputs_time, bwd_params_time, fwd_algo_s) =
                    time_cnn<uint16_t, uint16_t>(padded_k, padded_c, r, s, t, n, padded_h, padded_w, padded_d,pad_h, pad_w, pad_d, hstride, wstride,dstride, num_repeats, curand_gen, inference);
            }
        } else if ((precision == "int8") && inference) {
            if (!skip_kernel) {
                std::tie(fwd_time, bwd_inputs_time, bwd_params_time, fwd_algo_s) =
                    time_cnn<uint8_t, int>(padded_k, padded_c, r, s, t, n, padded_h, padded_w, padded_d, pad_h, pad_w, pad_d, hstride, wstride, dstride, num_repeats, curand_gen, inference);
            }
        } else {
            throw std::runtime_error(ss.str());
        }
        ...
        if (precision != "float")
            throw std::runtime_error(ss.str());
        std::tie(fwd_time, bwd_inputs_time, bwd_params_time, fwd_algo_s) =
            time_cnn<float, float>(k, c, r, s, t, n, h, w, d, pad_h, pad_w, pad_d, hstride, wstride, dstride, num_repeats, curand_gen, inference);
#endif
