...
public:
    ...
    TensorDescriptor5d() {}
    TensorDescriptor5d(const cudnnTensorFormat_t tensor_format,
        ...
        if (std::is_same<T, float>::value) {
            ...
            throw std::runtime_error("Unknown type in TensorDescriptor5d");
        }
	const int dim[5] = {n, c, d, h, w};
        cudnnTensorDescriptor_t * desc = new cudnnTensorDescriptor_t;
        CHECK_CUDNN_ERROR(cudnnCreateTensorDescriptor(desc));
        CHECK_CUDNN_ERROR(cudnnSetTensorNdDescriptorEx(*desc,
                                                     tensor_format,
                                                     type,
						     5,
                                                     dim));
...
public:
    ...
    FilterDescriptor5d(const cudnnTensorFormat_t tensor_format,
                       int k, int c, int h, int w, int d) {
        cudnnDataType_t type;
        if (std::is_same<T, float>::value) {
            type = CUDNN_DATA_FLOAT;
#if CUDNN_MAJOR >= 6
            ...
            throw std::runtime_error("Unknown type in FilterDescriptor5d");
        }

        cudnnFilterDescriptor_t * desc = new cudnnFilterDescriptor_t;
        CHECK_CUDNN_ERROR(cudnnCreateFilterDescriptor(desc));
	const int filter[5] = {k, c, d, h, w};
        CHECK_CUDNN_ERROR(cudnnSetFilterNdDescriptor(*desc, type, tensor_format, 5, filter));
...
public:
    ...
    ConvolutionDescriptor(int pad_h, int pad_w, int pad_d, int hstride, int wstride, int dstride) :
        ...
        const int pad[3] = {pad_h, pad_w, pad_d};
        const int stride[3] = {hstride, wstride, dstride};
