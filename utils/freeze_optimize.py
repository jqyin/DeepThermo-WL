...
def run_main(unused_args):
  ...
  if model.type == "SmartRedis":
      tensorflow.io.write_graph(graph_or_graph_def=gdef,  #opt_graph,
            logdir=".",
            name='%s/%s.pb'%(output_model_dir, output_model_name),
            as_text=False)
...
def parse_args():
  ...
  parser.add_argument(
      "--output_model_name",
      type=str,
      default="TF_SavedModel",
      help="name of TensorFlow \'SavedModel\'.")
  parser.add_argument(
      "--signature_key",
      type=str,
